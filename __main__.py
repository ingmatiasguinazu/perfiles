...
class FileSaveas(Transaction):
    ...
    def pretrans_validation(self):
        ...
        if len(self.controler.app_file.versions) > 1 :
            ...
            l_ret.append('IMPORTANTE! Este archivo contine historico de '
                    + 'versiones que no seran guardados en el nuevo archivo. ' 
                    + 'OK para continuar')
            return l_ret
    ...
    def execute(self):
        ...
        l_file_name = fdlg.asksaveasfilename(initialdir="c:\\", 
                title="[Guardar Como]", filetypes = (
                    ('Archivos Perfil', '*.per'), ('Todos', '*.*')))

        if l_file_name == '' or l_file_name == None:
...
class FileNew(Transaction):
    ...
    def execute(self):
        ...
        # Obtiene objeto seleccionado
        if self.controler.app_file.db_cnx != None:
...
class FileOpen(Transaction):
    ...
    def execute(self):
        ...
        # Obtiene objeto seleccionado
        l_file_name = fdlg.askopenfilename(initialdir="c:\\", 
                title="[Abrir Archivo]", filetypes = (
                    ('Archivos Perfil', '*.per'), ('Todos', '*.*')))
